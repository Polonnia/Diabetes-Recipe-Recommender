...
def calculate_nutrient_score(user_data, recipe_nutrition, meal_type):
    ...
    # 2. 计算每餐的能量需求
    meal_energy = calculate_meal_energy(tdee, meal_type)
    
    # 3. 计算每餐的营养需求
    meal_nutrient_needs = calculate_meal_nutrient_needs(meal_energy)
...
def calculate_meal_energy(tdee, meal_type):
    """
    计算每餐的能量需求。
    """
    meal_ratios = {
    ...
    return tdee * meal_ratios.get(meal_type, 0.3)

def calculate_meal_nutrient_needs(meal_energy):
    """
    计算每餐的营养需求。
    """
    return {
        "carb": meal_energy * 0.5 / 4,  # 碳水化合物占 50%，1g 碳水化合物 = 4 kcal
        "protein": meal_energy * 0.2 / 4,  # 蛋白质占 20%，1g 蛋白质 = 4 kcal
        "fat": meal_energy * 0.3 / 9,  # 脂肪占 30%，1g 脂肪 = 9 kcal
        "fiber": 10,  # 每餐膳食纤维需求（假设为 10g）
    }